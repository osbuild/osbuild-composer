...
func (impl *OSBuildJobImpl) getPulpClient(targetOptions *target.PulpOSTreeTargetOptions) (*pulp.Client, error) {
	// Read server configuration and credentials from the target options and
	// fall back to worker config if they are not set (targetOptions take precedent).
	// Mixing sources is allowed. For example, the server address can be
	// configured in the worker config while the targetOptions provide the
...
func (impl *OSBuildJobImpl) Run(job worker.Job) error {
	...
	for _, jobTarget := range jobArgs.Targets {
		...
		case *target.PulpOSTreeTargetOptions:
			targetResult = target.NewPulpOSTreeTargetResult(nil)
