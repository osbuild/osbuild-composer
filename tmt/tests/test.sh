...
function run_tests() {
	...
	elif [ "$TEST_CASE" = "edge-raw-image-fips" ]; then
		FIPS=true /usr/libexec/tests/osbuild-composer/ostree-raw-image.sh
	elif [ "$TEST_CASE" = "edge-simplified-installer" ]; then
		/usr/libexec/tests/osbuild-composer/ostree-simplified-installer.sh
	elif [ "$TEST_CASE" = "edge-simplified-installer-fips" ]; then
		FIPS=true /usr/libexec/tests/osbuild-composer/ostree-simplified-installer.sh
	...
	elif [ "$TEST_CASE" = "edge-vsphere-fips" ]; then
		FIPS=true /usr/libexec/tests/osbuild-composer/ostree-vsphere.sh
