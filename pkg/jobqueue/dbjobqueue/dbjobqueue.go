...
func (q *DBJobQueue) deleteJob(ctx context.Context, jobID uuid.UUID) error {
	conn, err := q.pool.Acquire(ctx)
	if err != nil {
		return err
	}
...
func (q *DBJobQueue) deleteJobDependencies(ctx context.Context, jobID, dependencyID uuid.UUID) error {
	conn, err := q.pool.Acquire(ctx)
	if err != nil {
		return err
	}
...
func (q *DBJobQueue) listJobs(ctx context.Context) (jobs []uuid.UUID, err error) {
	conn, err := q.pool.Acquire(ctx)
	if err != nil {
		return
	}
...
func (q *DBJobQueue) deleteJobs(ctx context.Context, conn connection, parent, id uuid.UUID) error {
	...
	if len(parent.String()) > 0 {
		err := q.deleteJobDependencies(ctx, parent, id)
		if err != nil {
			return err
		}
	}
	...
	return q.deleteJob(ctx, id) // Actual delete from the database
}
