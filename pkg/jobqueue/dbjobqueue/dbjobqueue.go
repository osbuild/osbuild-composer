...
func (q *DBJobQueue) listJobs() (jobs []uuid.UUID, err error) {
	conn, err := q.pool.Acquire(context.Background())
	if err != nil {
		return
	}
	...
	rows, err := conn.Query(context.Background(), sqlQueryListJobs)
	if err != nil {
		return
	}
	defer rows.Close()
...
func (q *DBJobQueue) DeleteJob(id uuid.UUID) error {
	conn, err := q.pool.Acquire(context.Background())
	if err != nil {
		return err
	}
	...
	return q.deleteJobs(conn, uuid.UUID{}, id)
}
...
func (q *DBJobQueue) deleteJobs(conn connection, parent, id uuid.UUID) error {
	...
	for _, d := range deps {
		_ = q.deleteJobs(conn, id, d) // Recursively delete dependencies
	}

	return q.deleteJob(id) // Actual delete from the database
}
