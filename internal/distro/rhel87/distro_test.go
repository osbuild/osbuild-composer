...
import (
	...
	"github.com/osbuild/osbuild-composer/internal/distro/rhel87"
)
...
func TestRhel87_ListArches(t *testing.T) {
	arches := rhel87.New().ListArches()
	assert.Equal(t, []string{"aarch64", "ppc64le", "s390x", "x86_64"}, arches)
}

func TestRhel87_GetArch(t *testing.T) {
	arches := []struct {
		name                  string
		errorExpected         bool
...
func TestRhel87_Name(t *testing.T) {
	distro := rhel87.New()
	assert.Equal(t, "rhel-87", distro.Name())
}

func TestRhel87_ModulePlatformID(t *testing.T) {
	distro := rhel87.New()
	assert.Equal(t, "platform:el8", distro.ModulePlatformID())
}

func TestRhel87_KernelOption(t *testing.T) {
	distro_test_common.TestDistro_KernelOption(t, rhel87.New())
}
