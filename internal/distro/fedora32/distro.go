...
func New(confPaths []string) *Fedora32 {
	...
	r.outputs["vhd"] = output{
		...
		KernelOptions: "ro biosdevname=0 rootdelay=320 console=ttyS0 earlyprintk=ttyS0 net.ifnames=0",
		Bootable:      true,
		DefaultSize:   2 * GigaByte,
		Assembler: func(uefi bool, size uint64) *pipeline.Assembler {
...
func (r *Fedora32) Pipeline(b *blueprint.Blueprint, additionalRepos []rpmmd.RepoConfig, checksums map[string]string, outputArchitecture, outputFormat string, size uint64) (*pipeline.Pipeline, error) {
...
func (r *Fedora32) buildPipeline(arch arch, checksums map[string]string) *pipeline.Pipeline {
	packages := []string{
		"dnf",
		"dosfstools",
		"e2fsprogs",
...
func (r *Fedora32) dnfStageOptions(arch arch, additionalRepos []rpmmd.RepoConfig, checksums map[string]string, packages, excludedPackages []string) *pipeline.DNFStageOptions {
	options := &pipeline.DNFStageOptions{
		ReleaseVersion:   "32",
		BaseArchitecture: arch.Name,
		ModulePlatformId: "platform:f32",
	}
...
func (r *Fedora32) userStageOptions(users []blueprint.UserCustomization) (*pipeline.UsersStageOptions, error) {
...
func (r *Fedora32) groupStageOptions(groups []blueprint.GroupCustomization) *pipeline.GroupsStageOptions {
...
func (r *Fedora32) firewallStageOptions(firewall *blueprint.FirewallCustomization) *pipeline.FirewallStageOptions {
...
func (r *Fedora32) systemdStageOptions(enabledServices, disabledServices []string, s *blueprint.ServicesCustomization) *pipeline.SystemdStageOptions {
	if s != nil {
...
func (r *Fedora32) fsTabStageOptions(uefi bool) *pipeline.FSTabStageOptions {
...
func (r *Fedora32) grub2StageOptions(kernelOptions string, kernel *blueprint.KernelCustomization, uefi bool) *pipeline.GRUB2StageOptions {
...
func (r *Fedora32) selinuxStageOptions() *pipeline.SELinuxStageOptions {
	return &pipeline.SELinuxStageOptions{
		FileContexts: "etc/selinux/targeted/contexts/files/file_contexts",
	}
}

func (r *Fedora32) qemuAssembler(format string, filename string, uefi bool, size uint64) *pipeline.Assembler {
...
func (r *Fedora32) tarAssembler(filename, compression string) *pipeline.Assembler {
...
func (r *Fedora32) rawFSAssembler(filename string, size uint64) *pipeline.Assembler {
	id, err := uuid.Parse("76a22bf4-f153-4541-b6c7-0332c0dfaeac")
	if err != nil {
		panic("invalid UUID")
	}
