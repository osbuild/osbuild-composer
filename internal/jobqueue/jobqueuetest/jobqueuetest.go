...
func testDeleteJobs(t *testing.T, q jobqueue.JobQueue) {
	...
	t.Run("koji job simulation", func(t *testing.T) {
		...
		kojiRoot := pushTestJob(t, q, "final", nil, []uuid.UUID{kojiInit, kojiManifest, kojiDepsolve}, "KojiJob")

		// Delete the koji job
		err := q.DeleteJob(kojiRoot)
		require.Nil(t, err)
		jobs, err := q.AllRootJobIDs()
		require.Nil(t, err)
		require.Equal(t, 0, len(jobs))
