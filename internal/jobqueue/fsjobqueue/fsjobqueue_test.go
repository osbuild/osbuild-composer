...
func TestAllRootJobIDs(t *testing.T) {
	...
	jid, err := q.Enqueue("oneRoot", nil, nil, "OneRootJob")
	require.Nil(t, err)
	rootJobs = append(rootJobs, jid)
	...
	require.Nil(t, err)
	jid, err = q.Enqueue("twoDeps", nil, []uuid.UUID{jid1, jid2}, "TwoDepJobs")
	require.Nil(t, err)
	rootJobs = append(rootJobs, jid)
	...
	require.Nil(t, err)
	jid, err = q.Enqueue("sharedDeps", nil, []uuid.UUID{jid1, jid3}, "SharedDepJobs")
	require.Nil(t, err)
	rootJobs = append(rootJobs, jid)
...
func TestRemoveJob(t *testing.T) {
	...
	jid2, err := q.Enqueue("twoDeps", nil, nil, "TwoDepJobs")
	require.Nil(t, err)
	jidRoot1, err := q.Enqueue("twoDeps", nil, []uuid.UUID{jid1, jid2}, "TwoDepJobs")
	require.Nil(t, err)

	// root with 2 dependencies, one shared with the previous root
	jid3, err := q.Enqueue("sharedDeps", nil, nil, "SharedDepJobs")
	require.Nil(t, err)
	jidRoot2, err := q.Enqueue("sharedDeps", nil, []uuid.UUID{jid1, jid3}, "SharedDepJobs")
	require.Nil(t, err)

	// This should only remove jidRoot1 and jid2, leaving jidRoot2, jid1, jid3
	...
	sortUUIDs(jobs)
	expected := []uuid.UUID{jidRoot2, jid1, jid3}
	sortUUIDs(expected)
