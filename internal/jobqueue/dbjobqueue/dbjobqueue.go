...
const (
	...
	sqlDequeue = `
		UPDATE jobs
		SET token = gen_random_uuid(), started_at = now()
		WHERE id = (
		  SELECT id
		  FROM ready_jobs
	...
	sqlFinishJob = `
		UPDATE jobs
		SET finished_at = now(), result = $1, token = $2
...
func (q *dbJobQueue) Dequeue(ctx context.Context, jobTypes []string) (uuid.UUID, uuid.UUID, []uuid.UUID, string, json.RawMessage, error) {
	...
	for {
		err = conn.QueryRow(ctx, sqlDequeue, jobTypes).Scan(&id, &token, &jobType, &args)
		if err == nil {
			break
		}
...
func (q *dbJobQueue) FinishJob(id uuid.UUID, result interface{}) error {
	...
	// todo return errnotexist as well if it can't find it
	// TODO check if job is not running first and return errnotrunning or errcanceled
	...
	tag, err = conn.Exec(context.Background(), sqlFinishJob, result, uuid.Nil, id)
	if err != nil {
		return fmt.Errorf("error finishing job %s: %v", id, err)
	}
...
func (q *dbJobQueue) IdFromToken(token uuid.UUID) (id uuid.UUID, err error) {
	...
	err = conn.QueryRow(context.Background(), sqlQueryId, token).Scan(&id)
	if err == pgx.ErrNoRows {
...
func (q *dbJobQueue) RefreshHeartbeat(token uuid.UUID) {
	...
	_, _ = conn.Exec(context.Background(), sqlRefreshHeartbeat, token)
}
