...
# Test the images
/usr/bin/time -v /usr/libexec/osbuild-composer-test/libvirt_test.sh qcow2

/usr/bin/time -v /usr/libexec/osbuild-composer-test/libvirt_test.sh openstack
...
if [[ "$DISTRO_CODE" == "rhel-84" || "$DISTRO_CODE" == "rhel-85" || "$DISTRO_CODE" == "centos-8" || "$DISTRO_CODE" == "rhel-90" ]]; then
  ...
  /usr/bin/time -v /usr/libexec/osbuild-composer-test/libvirt_test.sh qcow2 uefi
fi
