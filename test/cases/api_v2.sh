...
case $CLOUD_PROVIDER in
  "$CLOUD_PROVIDER_AWS")
    ...
    ;;
  "$CLOUD_PROVIDER_GCP")
    echo "GCP skipped for v2"
    exit 0
    ;;
  "$CLOUD_PROVIDER_AZURE")
    echo "Azure skipped for v2"
    exit 0
    ;;
...
function sendCompose() {
    ...
    OUTPUT=$(curl \
                 ...
                 --request POST \
                 --data @"$REQUEST_FILE" \
                 https://localhost/api/composer/v2/compose)
...
[ "$(curl \
    ...
    --data @"$REQUEST_FILE2" \
    https://localhost/api/composer/v2/compose)" = "400" ]
...
[ "$(curl \
    ...
    -H "Content-Type: application/json" \
    --data @"$REQUEST_FILE2" \
    https://localhost/api/composer/v2/compose)" = "500" ]
